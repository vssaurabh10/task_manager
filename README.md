...
2. Create a Railway MongoDB database or use MongoDB Atlas.
3. Create a Railway service for `backend`.
4. Set root directory to `backend`.
5. Add `MONGO_URI`, `JWT_SECRET`, `JWT_EXPIRES_IN`, `CLIENT_URL`, and `NODE_ENV=production`.
6. Build command: `npm install && npm run build`.
7. Start command: `npm start`.

Frontend:

1. Deploy `frontend` to Vercel or Railway.
2. Set `VITE_API_URL` to the deployed backend URL plus `/api`.
3. Set backend `CLIENT_URL` to the deployed frontend URL.
